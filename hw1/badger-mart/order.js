...
function calculateSubtotal() {
  let sum = 0;
  for(let i of ITEMS){
    const val = document.getElementById(i + "-quantity").value;
    if(/^\d+$/.test(val)){
      sum +=
        parseFloat(document.getElementById(i+"-price").innerText) * 
        document.getElementById(i+"-quantity").value;
    }else{
      alert("Please input Quantity Needed properly.");
      return NaN;
    }
  }
...
function calculateSalesTax() {
  const state = document.getElementById("state-tax").value;
	return getSalesTaxRateForState(state) * calculateSubtotal(); //  calculateSalesTax
}
